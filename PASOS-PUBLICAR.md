...
- **Punto de venta que descuenta stock automáticamente al confirmar una venta**, con datos del cliente (nombre, DNI, WhatsApp — opcionales) guardados en cada venta, y escaneo de código de barras con la cámara (Android/Chrome)
- **Log de ventas** (Panel → Ventas): fecha, hora, productos vendidos con talle/color/cantidad, total y datos del cliente de cada venta
- **Edición y borrado de productos y variantes** desde el panel, con subida de foto de producto
...
**"permission denied for table products" al crear/ver productos**
Si corriste el `supabase-schema.sql` antes de esta actualización, te faltan los permisos base. Solución: abrí `fix-permisos.sql` (incluido en este zip), copiá todo su contenido, pegalo en Supabase → SQL Editor → Run. Es un script de una sola vez, no rompe nada de lo que ya tenés. Si descargaste el zip después de esta fecha, el `supabase-schema.sql` ya lo incluye y no hace falta este paso aparte.

**Si ya tenías la app instalada antes de esta actualización (edición de productos, fotos, log de ventas)**
Corré también `fix-ventas-cliente.sql` en Supabase → SQL Editor → Run (una sola vez). Agrega las columnas necesarias para guardar los datos del cliente en cada venta del POS. Si es una instalación nueva, ya está incluido en `supabase-schema.sql`.
