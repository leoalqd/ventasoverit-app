...
/**
 * Confirma una venta: crea el registro de venta, sus items, descuenta stock
 * de cada variante y deja el movimiento de stock registrado.
 */
export async function confirmSale(cart, userId) {
  const subtotal = cart.reduce((sum, i) => sum + i.price * i.qty, 0);

  const { data: sale, error: saleError } = await supabase
    .from('sales')
    .insert({ user_id: userId, subtotal, discount: 0, total: subtotal })
    .select()
    .single();
